...
class SampleAssistant(object):
    ...
    """def change_stream(self):
        ...
        self.conversation_stream = audio_helpers.ConversationStream(audio_source, audio_source, audio_helpers.DEFAULT_AUDIO_ITER_SIZE, audio_helpers.DEFAULT_AUDIO_SAMPLE_WIDTH)
      """  
            ...
            if resp.speech_results:
                ...
                if wiki_response is None:
                    ...
                    if len(wiki_response) > 2:
                        if(counter == 0):
                            texttospeech.synthesize_ssml("<speak> {} </speak".format(wiki_response))
                            counter += 1;
                            temp_data = resp.audio_out.audio_data;
                            resp.audio_out.audio_data = open('output', 'rb').read()
...
def main(api_endpoint, credentials, project_id,
    ...
    with SampleAssistant(lang, device_model_id, device_id,
        ...
        if input_audio_file or output_audio_file:
            ...
            assistant.conversation_stream = audio_helpers.ConversationStream(
                source=audio_source,
                sink=audio_sink,
                iter_size=audio_iter_size,
                sample_width=audio_sample_width,
